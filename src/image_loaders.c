...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/13 13:38:24 by ancardos          #+#    #+#             */
/*   Updated: 2024/08/13 13:38:26 by ancardos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*img = mlx_xpm_file_to_image(mlx, path,
...
void	create_images(t_data *data, int i, t_sprite_dim dim)
{
	...
	if (!data->img_player_down[i] || !data->img_player_left[i]
		|| !data->img_player_right[i] || !data->img_player_up[i])
	{
		printf("Error creating images\n");
		exit(1);
	}
}

void	copy_sprite_part(t_image_data *img_data,
		int i, t_sprite_dim dim, int y_offset)
{
...
void	copy_sprite_images(t_data *data, t_image_data *img_data,
		int i, t_sprite_dim dim)
{
